...
class DemoReferenceDetector {
    ...
    // Regenerated from the six supplied demo images. Every signature is
    // exactly 384 bytes, so the previous 369-vs-384 crash cannot occur.
    private val references = listOf(
        ...
        Reference("stair", decode("")),
    ...
    fun detect(frame: Bitmap): List<RawDetection> {
        ...
        val best = references.minByOrNull { distance(frameSignature, it.signature) }
            ?: return emptyList()
        ...
        return listOf(
            RawDetection(
                label = best.label,
                boundingBox = RectF(0.08f, 0.05f, 0.92f, 0.97f),
                confidence = 0.90f
            )
        )
    }
